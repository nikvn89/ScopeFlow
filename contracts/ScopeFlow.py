...
class ScopeGuard(gl.Contract):
    ...
    # new snapshot for each fully approved extension. Snapshot text is never
    # overwritten after creation.
    project_scope_version_counts: TreeMap[u256, u256]
    scope_version_scopes: TreeMap[str, str]
    scope_version_previous: TreeMap[str, u256]
    scope_version_request_ids: TreeMap[str, u256]
    scope_version_effective_at: TreeMap[str, u256]
    scope_version_client_approved: TreeMap[str, bool]
    scope_version_contractor_approved: TreeMap[str, bool]
    ...
    def _scope_version_key(
        ...
        if version <= 0:
            raise gl.vm.UserError("Invalid scope version")
        return str(project_id) + ":" + str(version)

    def _scope_version_exists(
        self,
        project_key: u256,
        version: int,
    ) -> bool:
        ...
        return version > 0 and version <= count
...
"""
    ...
    def accept_project(
        ...
        # V1 becomes effective only when the named contractor accepts the
        # exact client-committed scope. This creates the first immutable
        # effective-scope snapshot.
        version_key = self._scope_version_key(project_id, 1)
        self.project_scope_version_counts[project_key] = u256(1)
        self.scope_version_scopes[version_key] = str(
            self.project_scopes.get(project_key, "")
        )
        self.scope_version_previous[version_key] = u256(0)
        self.scope_version_request_ids[version_key] = u256(0)
        self.scope_version_effective_at[version_key] = u256(now)
        self.scope_version_client_approved[version_key] = True
        self.scope_version_contractor_approved[version_key] = True
    ...
    def approve_extension(
        ...
        if client_ok and contractor_ok:
            ...
            # back to the request that created the new version.
            version_key = self._scope_version_key(
                project_id,
                new_version,
            )
            self.project_scope_version_counts[
                project_key
            ] = u256(new_version)
            self.scope_version_scopes[version_key] = new_scope
            self.scope_version_previous[version_key] = u256(
                active_version
            )
            self.scope_version_request_ids[version_key] = u256(
                request_id
            )
            self.scope_version_effective_at[version_key] = u256(now)
            self.scope_version_client_approved[version_key] = client_ok
            self.scope_version_contractor_approved[version_key] = contractor_ok
    ...
    def get_scope_version(
        ...
        if not self._scope_version_exists(project_key, version):
            raise gl.vm.UserError("Scope version not found")
        ...
        request_id = int(
            self.scope_version_request_ids.get(
                version_key,
                u256(0),
            )
        )
        scope_text = str(
            self.scope_version_scopes.get(
                version_key,
                "",
            )
        )
        ...
        return json.dumps(
            {
                ...
                "previous_version": int(
                    self.scope_version_previous.get(
                        version_key,
                        u256(0),
                    )
                ),
                ...
                "effective_at": int(
                    self.scope_version_effective_at.get(
                        version_key,
                        u256(0),
                    )
                ),
                "client_approved": bool(
                    self.scope_version_client_approved.get(
                        version_key,
                        False,
                    )
                ),
                "contractor_approved": bool(
                    self.scope_version_contractor_approved.get(
                        version_key,
                        False,
                    )
                ),
    ...
    def get_scope_versions(
        ...
        if from_version <= 0:
            raise gl.vm.UserError("Invalid start version")

        if count <= 0 or count > 20:
            raise gl.vm.UserError("Invalid count")
        ...
        if from_version <= total:
            current = from_version
            end_version = from_version + count - 1
            if end_version > total:
                end_version = total

            while current <= end_version:
                version_key = self._scope_version_key(
                    project_id,
                    current,
                )
